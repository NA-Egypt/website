<template>
  <div class="service-bodies-container" :dir="isRtl ? 'rtl' : 'ltr'">
    <!-- Top Summary KPI Metric Cards -->
    <div class="row g-3 mb-4">
      <!-- Total Service Bodies -->
      <div class="col-12 col-md-6">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.totalServiceBodies || 'Total Service Bodies' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.total_service_bodies }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-12 col-md-6">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.hostedGroups || 'Linked Groups' }}</div>
          ...
          <div class="search-box-wrapper">
            <div class="search-box">
              ...
              <input
                ...
                v-model="searchQuery"
                class="search-input"
                :placeholder="labels.searchPlaceholder || 'Search service bodies...'"
                @input="onSearchInput"
              />
              <button
                v-if="searchQuery"
          ...
          <table class="table table-hover align-middle mb-0 custom-datatable">
              ...
              <tr class="table-header-row">
                <th scope="col" class="ps-4 py-3" style="min-width: 240px;" @click="setSort('ar_name')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.serviceBodyName || 'Service Body' }}</span>
                    <i class="bi sort-icon" :class="getSortIcon('ar_name')"></i>
                  </div>
                </th>
                <th scope="col" class="py-3" style="min-width: 200px;">
                  {{ labels.meetingDayTime || 'Day & Time' }}
                </th>
                <th scope="col" class="py-3" style="min-width: 220px;">
                  {{ labels.meetingAddress || 'Location / Address' }}
                </th>
                <th scope="col" class="py-3 text-center" style="min-width: 140px;">
                  {{ labels.hostedGroups || 'Groups' }}
                </th>
            ...
            <tbody>
              ...
              <tr v-else v-for="item in rows" :key="item.id" class="table-data-row">
                ...
                <td class="ps-4 py-3">
                  <div class="d-flex align-items-center gap-2">
                    <div class="sb-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 38px; height: 38px; background: rgba(13, 110, 253, 0.12); color: #0d6efd;">
                ...
                <td class="py-3">
                  <div class="d-flex flex-column gap-1">
                    <span v-if="item.day" class="badge rounded-pill bg-light text-dark border align-self-start px-2 py-1 small">
                      <i class="bi bi-calendar3 me-1 text-primary"></i>
                      {{ isRtl ? (item.day.ar_name || item.day.en_name) : (item.day.en_name || item.day.ar_name) }}
                    </span>
                    <div v-if="item.start_time" class="small text-muted d-flex align-items-center gap-1">
                ...
                <td class="py-3">
                  <div class="d-flex align-items-center gap-2">
                    <div class="text-truncate small" style="max-width: 200px;" :title="item.location">
                    ...
                    <a
                      ...
                      :title="labels.viewOnMap || 'View on Google Maps'"
                    >
                ...
                <td class="py-3 text-center">
                  <span class="badge rounded-pill px-3 py-2 fw-semibold" :class="item.groups_count > 0 ? 'bg-success-subtle text-success' : 'bg-secondary-subtle text-secondary'">
                ...
                <td class="pe-4 py-3 text-end">
                  <div class="d-flex align-items-center justify-content-end gap-1">
                    ...
                    <a
                      ...
                      :title="labels.viewDetails || 'Details'"
                    >
            ...
            >
              <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-start mb-2">
                  <div class="d-flex align-items-center gap-2">
                    <div class="sb-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 36px; height: 36px; background: rgba(13, 110, 253, 0.12); color: #0d6efd;">
                      <i class="bi bi-building"></i>
                    </div>
                    <div>
                      <div class="fw-bold" style="color: var(--text-primary);">
                  ...
                  <span class="badge rounded-pill px-2 py-1 small" :class="item.groups_count > 0 ? 'bg-success-subtle text-success' : 'bg-secondary-subtle text-secondary'">
                    <i class="bi bi-people me-1"></i>
                    {{ item.groups_count || 0 }}
                  </span>
                </div>

                <div v-if="item.day" class="mb-2 small text-muted">
                  <i class="bi bi-calendar3 me-1 text-primary"></i>
                  {{ isRtl ? (item.day.ar_name || item.day.en_name) : (item.day.en_name || item.day.ar_name) }}
                  <span v-if="item.start_time" class="ms-1">({{ item.start_time }})</span>
                </div>

                <div v-if="item.location" class="mb-2 small text-muted d-flex align-items-center gap-1">
            ...
            <div class="d-flex align-items-center gap-2">
              ...
              <div>
                <h5 class="modal-title fw-bold mb-0" style="color: var(--text-primary);">
                  {{ labels.serviceBodyDetails || 'Service Body Details' }}
                </h5>
            ...
            <div v-if="selectedItem" class="row g-3">
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.serviceBodyName || 'Name (AR / EN)' }}</div>
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.meetingDayTime || 'Day & Time' }}</div>
                  <div class="fw-bold fs-6 mb-1" style="color: var(--text-primary);">
                    {{ selectedItem.day ? (isRtl ? (selectedItem.day.ar_name || selectedItem.day.en_name) : (selectedItem.day.en_name || selectedItem.day.ar_name)) : 'N/A' }}
                  </div>
                  <div v-if="selectedItem.start_time" class="small text-muted">
                    {{ selectedItem.start_time }} {{ selectedItem.end_time ? '- ' + selectedItem.end_time : '' }}
                  </div>
                </div>
              </div>
              ...
              <div class="col-12">
                <div class="p-3 rounded-3 detail-info-box">
                  <div class="small text-muted fw-semibold mb-2">{{ labels.meetingAddress || 'Meeting Address' }}</div>
                  <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <a
                      v-if="hasCoordinates(selectedItem)"
                      :href="getMapUrl(selectedItem)"
                      target="_blank"
                      class="btn btn-sm btn-outline-primary rounded-pill px-3"
                    >
                      <i class="bi bi-map me-1"></i>
                      {{ labels.viewOnMap || 'View on Google Maps' }}
                    </a>
                  </div>
                </div>
              </div>
              ...
              <!-- Linked Groups count -->
              <div class="col-12">
                <div class="p-3 rounded-3 detail-info-box d-flex justify-content-between align-items-center">
                  <div>
                    <div class="small text-muted fw-semibold mb-1">{{ labels.hostedGroups || 'Linked Groups' }}</div>
                    <div class="fw-bold fs-5 text-success">
                      {{ selectedItem.groups_count || 0 }}
                    </div>
                  </div>
...
const kpiData = reactive({
  total_service_bodies: props.initialKpiStats?.total_service_bodies || 0,
...
const hasActiveFilters = computed(() => {
  return searchQuery.value !== '' || selectedDay.value !== '';
});
...
const getShowUrl = (id) => {
  return props.showRouteTemplate ? props.showRouteTemplate.replace('__ID__', id) : '#';
};

const hasCoordinates = (item) => {
  return !!(item && item.latitude && item.longitude);
};
...
const fetchData = async () => {
  ...
  try {
    ...
    if (selectedDay.value) {
      params.day_id = selectedDay.value;
    }
    ...
    console.error('Failed to load service bodies:', error);
  } finally {
    loading.value = false;
  }
};
...
const confirmDelete = async (item) => {
  const confirmMsg = (props.labels.confirmDelete || 'Are you sure you want to delete service body ":name"?')
  ...
  if (result.isConfirmed) {
    try {
      ...
      if (res.data?.success || res.status === 200 || res.status === 204) {
        Swal.fire({
          icon: 'success',
          title: props.labels.deletedSuccess || 'Service body deleted successfully.',
          timer: 2000,
          showConfirmButton: false
        });
        kpiData.total_service_bodies = Math.max(0, kpiData.total_service_bodies - 1);
        fetchData();
      }
    } catch (err) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: err.response?.data?.message || 'Failed to delete service body.'
      });
    }
  }
};
