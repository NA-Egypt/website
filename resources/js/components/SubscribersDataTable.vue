<template>
  <div class="subscribers-container" :dir="isRtl ? 'rtl' : 'ltr'">
      ...
      <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.totalSubscribers || 'Total Subscribers' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.total_subscribers }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.verifiedSubscribers || 'Verified' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.verified_subscribers }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.unverifiedSubscribers || 'Unverified' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.unverified_subscribers }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.verificationRate || 'Verification Rate' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.verification_rate }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="card border-0 shadow-sm mb-4 glass-card">
      <div class="card-body p-3 d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div class="d-flex flex-wrap align-items-center gap-2 flex-grow-1">
          ...
          <div class="search-box-wrapper">
            <div class="search-box">
              ...
              <input
                ...
                v-model="searchQuery"
                class="search-input"
                :placeholder="labels.searchPlaceholder || 'Search subscribers by email...'"
                @input="onSearchInput"
              />
              <button
                v-if="searchQuery"
          ...
          <div class="filter-wrapper">
            <select v-model="selectedStatus" @change="fetchData" class="form-select form-select-sm filter-select">
              ...
              <option value="verified">{{ labels.verifiedSubscribers || 'Verified' }}</option>
              <option value="unverified">{{ labels.unverifiedSubscribers || 'Unverified' }}</option>
            </select>
          </div>
        ...
        <!-- Special Action Buttons (Export, Batch Verify, Create) -->
        <div class="d-flex align-items-center flex-wrap gap-2">
          <!-- Export CSV -->
          <a
            v-if="exportRoute"
            :href="exportRoute"
            class="btn btn-outline-secondary btn-sm px-3 d-inline-flex align-items-center gap-2 fw-semibold"
          >
          ...
          <!-- Batch Verify SMTP -->
          <button
            v-if="getIdsUrl && verifyBatchUrl"
            type="button"
            class="btn btn-outline-info btn-sm px-3 d-inline-flex align-items-center gap-2 fw-semibold"
            @click="startBatchVerify"
            :disabled="verifyingBatch"
          >
            <i class="bi bi-shield-check"></i>
            {{ labels.batchVerifySmtp || 'Batch Verify SMTP' }}
          </button>
          ...
          <table class="table table-hover align-middle mb-0 custom-datatable">
            <thead>
              <tr class="table-header-row">
                <th scope="col" class="ps-4 py-3" style="min-width: 280px;" @click="setSort('email')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.email || 'Email' }}</span>
                    <i class="bi sort-icon" :class="getSortIcon('email')"></i>
                  </div>
                </th>
                <th scope="col" class="py-3 text-center" style="min-width: 140px;">
                  {{ labels.status || 'Status' }}
                </th>
                <th scope="col" class="py-3" style="min-width: 180px;" @click="setSort('created_at')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.createdAt || 'Subscribed Date' }}</span>
                    <i class="bi sort-icon" :class="getSortIcon('created_at')"></i>
                  </div>
                </th>
              ...
              <tr v-else v-for="item in rows" :key="item.id" class="table-data-row">
                ...
                <td class="ps-4 py-3">
                  <div class="d-flex align-items-center gap-2">
                    <div class="sub-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 38px; height: 38px; background: rgba(13, 110, 253, 0.12); color: #0d6efd;">
                ...
                <td class="py-3 text-center">
                  <span v-if="item.is_verified" class="badge rounded-pill bg-success-subtle text-success px-3 py-1-5 fw-semibold">
                    <i class="bi bi-patch-check me-1"></i>
                    {{ labels.verifiedSubscribers || 'Verified' }}
                  </span>
                  <span v-else class="badge rounded-pill bg-warning-subtle text-warning-emphasis px-3 py-1-5 fw-semibold">
                    <i class="bi bi-hourglass-split me-1"></i>
                    {{ labels.unverifiedSubscribers || 'Unverified' }}
                  </span>
                </td>
                ...
                <td class="pe-4 py-3 text-end">
                  <div class="d-flex align-items-center justify-content-end gap-1">
                    ...
                    <!-- Toggle Verification -->
                    <form v-if="toggleRouteTemplate" :action="getToggleUrl(item.id)" method="POST" class="d-inline">
                      <input type="hidden" name="_token" :value="csrfToken">
                      <button
                        type="submit"
                        class="btn btn-sm btn-icon-action"
                        :class="item.is_verified ? 'btn-outline-warning' : 'btn-outline-success'"
                        :title="item.is_verified ? (labels.unverifySelected || 'Unverify') : (labels.verifySelected || 'Verify')"
                      >
                        <i :class="item.is_verified ? 'bi bi-x-circle' : 'bi bi-check-circle'"></i>
                      </button>
                    </form>
            ...
            >
              <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-start mb-2">
                  <div class="d-flex align-items-center gap-2">
                    <div class="sub-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 36px; height: 36px; background: rgba(13, 110, 253, 0.12); color: #0d6efd;">
                      <i class="bi bi-envelope"></i>
                    </div>
                    <div>
                      <div class="fw-bold small text-truncate" style="max-width: 190px; color: var(--text-primary);">{{ item.email }}</div>
                      <div class="text-muted" style="font-size: 0.75rem;">{{ item.created_at_formatted }}</div>
                    </div>
                  </div>
                  <span v-if="item.is_verified" class="badge rounded-pill bg-success-subtle text-success px-2 py-1 small">
                ...
                <div class="d-flex align-items-center justify-content-end gap-2 pt-2 border-top mt-2">
                  ...
                  <form v-if="toggleRouteTemplate" :action="getToggleUrl(item.id)" method="POST" class="d-inline">
                    ...
                    <button
                      ...
                      :class="item.is_verified ? 'btn-outline-warning' : 'btn-outline-success'"
                    >
                      {{ item.is_verified ? (labels.unverifySelected || 'Unverify') : (labels.verifySelected || 'Verify') }}
                    </button>
                  </form>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg modal-glassmorphic">
            ...
            <div class="d-flex align-items-center gap-2">
              ...
              <div>
                <h5 class="modal-title fw-bold mb-0" style="color: var(--text-primary);">
                  {{ labels.subscriberDetails || 'Subscriber Details' }}
                </h5>
                <div class="small text-muted">{{ selectedItem?.email }}</div>
              </div>
            </div>
            ...
            <div v-if="selectedItem" class="row g-3">
              <div class="col-12">
                <div class="p-3 rounded-3 detail-info-box">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.email || 'Email' }}</div>
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.status || 'Status' }}</div>
                  <div class="mt-1">
                    <span v-if="selectedItem.is_verified" class="badge rounded-pill bg-success-subtle text-success px-3 py-1-5 fw-semibold">
                      <i class="bi bi-patch-check me-1"></i>
                      {{ labels.verifiedSubscribers || 'Verified' }}
                    </span>
                    <span v-else class="badge rounded-pill bg-warning-subtle text-warning-emphasis px-3 py-1-5 fw-semibold">
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.createdAt || 'Subscribed Date' }}</div>
                  <div class="small fw-semibold mt-1" style="color: var(--text-primary);">
          ...
          <div class="modal-footer border-top py-3 px-4 d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-2">
              <form v-if="toggleRouteTemplate && selectedItem" :action="getToggleUrl(selectedItem.id)" method="POST" class="d-inline">
                <input type="hidden" name="_token" :value="csrfToken">
                <button
                  type="submit"
                  class="btn btn-sm px-3 fw-semibold"
                  :class="selectedItem.is_verified ? 'btn-outline-warning' : 'btn-outline-success'"
                >
                  {{ selectedItem.is_verified ? (labels.unverifySelected || 'Unverify') : (labels.verifySelected || 'Verify') }}
                </button>
              </form>
            </div>
        ...
        <div class="modal-content border-0 shadow-lg modal-glassmorphic">
          <div class="modal-header border-bottom py-3 px-4">
            <div class="d-flex align-items-center gap-2">
              ...
              <h5 class="modal-title fw-bold mb-0" style="color: var(--text-primary);">
                {{ labels.batchVerificationProgress || 'Batch Verification Progress' }}
              </h5>
            </div>
            <button
              v-if="!verifyingBatch"
          ...
          <div class="modal-body p-4 text-center">
            <div class="mb-3">
              <div class="fs-4 fw-bold mb-1" style="color: var(--text-primary);">
                {{ batchProgress }}%
              </div>
              <div class="small text-muted">
                {{ batchProcessed }} / {{ batchTotal }} {{ labels.processed || 'processed' }} ({{ batchDeleted }} {{ labels.deleted || 'deleted invalid' }})
              </div>
            </div>
            ...
            <div class="small text-muted">
              {{ verifyingBatch ? (labels.verifyingViaSmtp || 'Verifying unverified subscribers via SMTP...') : (labels.batchComplete || 'Batch Verification Complete') }}
            </div>
          </div>

          <div class="modal-footer border-top py-3 px-4 d-flex justify-content-end">
            <button
              type="button"
              class="btn btn-secondary btn-sm px-3"
              :disabled="verifyingBatch"
              @click="finishBatchVerify"
            >
              {{ labels.close || 'Close' }}
            </button>
          </div>
...
const kpiData = reactive({
  total_subscribers: props.initialKpiStats?.total_subscribers || 0,
  verified_subscribers: props.initialKpiStats?.verified_subscribers || 0,
  unverified_subscribers: props.initialKpiStats?.unverified_subscribers || 0,
  verification_rate: props.initialKpiStats?.verification_rate || '0%'
});
...
const getToggleUrl = (id) => {
  return props.toggleRouteTemplate ? props.toggleRouteTemplate.replace('__ID__', id) : '#';
};
...
const startBatchVerify = async () => {
  try {
    const res = await axios.get(props.getIdsUrl);
    const ids = res.data;
    if (!ids || ids.length === 0) {
      Swal.fire({
        icon: 'info',
        title: props.labels.batchComplete || 'Verification',
        text: props.labels.noUnverifiedSubscribers || 'No unverified subscribers found to verify.'
      });
      return;
    }
    ...
    for (let i = 0; i < ids.length; i += chunkSize) {
      ...
      const batchRes = await axios.post(props.verifyBatchUrl, {
      ...
      batchProcessed.value += (batchRes.data.processed || chunk.length);
      batchDeleted.value += (batchRes.data.deleted || 0);
    }
    ...
    Swal.fire({
      icon: 'error',
      title: 'Error',
      text: err.response?.data?.message || 'Batch verification failed.'
    });
  }
};

const finishBatchVerify = () => {
  showBatchModal.value = false;
  fetchData();
};

const confirmDelete = async (item) => {
  const confirmMsg = (props.labels.confirmDelete || 'Are you sure you want to delete subscriber ":email"?')
    .replace(':email', item.email || item.id);
  ...
  if (result.isConfirmed) {
    try {
      ...
      if (res.data?.success || res.status === 200 || res.status === 204) {
        Swal.fire({
          icon: 'success',
          title: props.labels.deletedSuccess || 'Subscriber deleted successfully.',
          timer: 2000,
          showConfirmButton: false
        });
        kpiData.total_subscribers = Math.max(0, kpiData.total_subscribers - 1);
        fetchData();
      }
    } catch (err) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: err.response?.data?.message || 'Failed to delete subscriber.'
      });
    }
  }
};
