<template>
  <div class="cities-container" :dir="isRtl ? 'rtl' : 'ltr'">
      ...
      <div class="col-12 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.totalCities || 'Total Cities' }}</div>
      ...
      <div class="col-6 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.linkedNeighborhoods || 'Linked Neighborhoods' }}</div>
    ...
    <div class="card border-0 shadow-sm mb-4 glass-card">
      <div class="card-body p-3 d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div class="d-flex flex-wrap align-items-center gap-2 flex-grow-1">
          ...
          <div class="search-box-wrapper">
            <div class="search-box">
              ...
              <input
                ...
                v-model="searchQuery"
                class="search-input"
                :placeholder="labels.searchPlaceholder || 'Search cities by name...'"
                @input="onSearchInput"
              />
              <button
                v-if="searchQuery"
          ...
          <!-- Has Neighborhoods Filter -->
          <div class="filter-wrapper">
            <select v-model="selectedHasNeighborhoods" @change="fetchData" class="form-select form-select-sm filter-select">
              <option value="">{{ labels.hasNeighborhoods || 'Has Neighborhoods' }}</option>
              <option value="yes">{{ labels.withNeighborhoods || 'With Neighborhoods' }}</option>
              <option value="no">{{ labels.withoutNeighborhoods || 'Without Neighborhoods' }}</option>
            </select>
          </div>
          ...
          <table class="table table-hover align-middle mb-0 custom-datatable">
              ...
              <tr class="table-header-row">
                <th scope="col" class="ps-4 py-3" style="min-width: 250px;" @click="setSort('ar_name')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.cityNameAr || 'Arabic Name' }}</span>
                    <i class="bi sort-icon" :class="getSortIcon('ar_name')"></i>
                  </div>
                </th>
                <th scope="col" class="py-3" style="min-width: 250px;" @click="setSort('en_name')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.cityNameEn || 'English Name' }}</span>
                    <i class="bi sort-icon" :class="getSortIcon('en_name')"></i>
                  </div>
                </th>
                <th scope="col" class="py-3 text-center" style="min-width: 170px;">
                  {{ labels.linkedNeighborhoods || 'Neighborhoods' }}
                </th>
            ...
            <tbody>
              ...
              <tr v-else v-for="item in rows" :key="item.id" class="table-data-row">
                ...
                <td class="ps-4 py-3">
                  <div class="d-flex align-items-center gap-2">
                    <div class="city-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 38px; height: 38px; background: rgba(13, 110, 253, 0.12); color: #0d6efd;">
                ...
                <!-- Linked Neighborhoods Count -->
                <td class="py-3 text-center">
                  <span class="badge rounded-pill px-3 py-2 fw-semibold" :class="item.neighborhoods_count > 0 ? 'bg-info-subtle text-info' : 'bg-secondary-subtle text-secondary'">
                    <i class="bi bi-geo-alt me-1"></i>
                    {{ item.neighborhoods_count || 0 }}
                  </span>
                </td>
            ...
            >
              <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-start mb-2">
                  <div class="d-flex align-items-center gap-2">
                    <div class="city-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 36px; height: 36px; background: rgba(13, 110, 253, 0.12); color: #0d6efd;">
                      <i class="bi bi-buildings"></i>
                    </div>
                    <div>
                  ...
                  <span class="badge rounded-pill px-2 py-1 small" :class="item.neighborhoods_count > 0 ? 'bg-info-subtle text-info' : 'bg-secondary-subtle text-secondary'">
                    <i class="bi bi-geo-alt me-1"></i>
                    {{ item.neighborhoods_count || 0 }}
                  </span>
                </div>
            ...
            <div class="d-flex align-items-center gap-2">
              ...
              <div>
                <h5 class="modal-title fw-bold mb-0" style="color: var(--text-primary);">
                  {{ labels.cityDetails || 'City Details' }}
                </h5>
            ...
            <div v-if="selectedItem" class="row g-3">
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.cityNameAr || 'Arabic Name' }}</div>
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.cityNameEn || 'English Name' }}</div>
                  <div class="fw-bold fs-6" style="color: var(--text-primary);">
              ...
              <div class="col-12">
                <div class="p-3 rounded-3 detail-info-box d-flex justify-content-between align-items-center">
                  <div>
                    <div class="small text-muted fw-semibold mb-1">{{ labels.linkedNeighborhoods || 'Linked Neighborhoods' }}</div>
                    <div class="fw-bold fs-5 text-info">
                      {{ selectedItem.neighborhoods_count || 0 }}
                    </div>
                  </div>
                  <div class="badge bg-info-subtle text-info p-2 rounded-3">
...
const selectedHasNeighborhoods = ref('');
...
const hasActiveFilters = computed(() => {
  return searchQuery.value !== '' || selectedHasNeighborhoods.value !== '';
});

const resetAllFilters = () => {
  searchQuery.value = '';
  selectedHasNeighborhoods.value = '';
  currentPage.value = 1;
  fetchData();
};
...
const fetchData = async () => {
  ...
  try {
    ...
    if (selectedHasNeighborhoods.value) {
      params.has_neighborhoods = selectedHasNeighborhoods.value;
    }
...
const confirmDelete = async (item) => {
  const confirmMsg = (props.labels.confirmDelete || 'Are you sure you want to delete city ":name"?')
  ...
  if (result.isConfirmed) {
    try {
      ...
      if (res.data?.success || res.status === 200 || res.status === 204) {
        Swal.fire({
          icon: 'success',
          title: props.labels.deletedSuccess || 'City deleted successfully.',
          timer: 2000,
          showConfirmButton: false
        });
        kpiData.total_cities = Math.max(0, kpiData.total_cities - 1);
        fetchData();
      }
    } catch (err) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: err.response?.data?.message || 'Failed to delete city.'
      });
    }
  }
};
