<template>
  <div class="topics-container" :dir="isRtl ? 'rtl' : 'ltr'">
      ...
      <div class="col-12 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.totalTopics || 'Total Topics' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.total_topics }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-6 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.linkedMeetings || 'Linked Meetings' }}</div>
      ...
      <div class="col-6 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.standardTopics || 'Active Topics' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.standard_topics }}</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="d-flex flex-wrap align-items-center gap-2 flex-grow-1">
          ...
          <div class="search-box-wrapper">
            <div class="search-box">
              ...
              <input
                ...
                v-model="searchQuery"
                class="search-input"
                :placeholder="labels.searchPlaceholder || 'Search topics...'"
                @input="onSearchInput"
              />
              <button
                v-if="searchQuery"
          ...
          <div class="filter-wrapper">
            <select v-model="selectedHasMeetings" @change="fetchData" class="form-select form-select-sm filter-select">
              <option value="">{{ labels.hasMeetings || 'Has Meetings' }}</option>
              <option value="yes">{{ labels.withMeetings || 'With Meetings' }}</option>
              <option value="no">{{ labels.withoutMeetings || 'Without Meetings' }}</option>
            </select>
          </div>
          ...
          <table class="table table-hover align-middle mb-0 custom-datatable">
            <thead>
              <tr class="table-header-row">
                <th scope="col" class="ps-4 py-3" style="min-width: 260px;" @click="setSort('ar_name')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.topicNameAr || 'Topic (AR)' }}</span>
                    <i class="bi sort-icon" :class="getSortIcon('ar_name')"></i>
                  </div>
                </th>
                <th scope="col" class="py-3" style="min-width: 260px;" @click="setSort('en_name')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.topicNameEn || 'Topic (EN)' }}</span>
                    <i class="bi sort-icon" :class="getSortIcon('en_name')"></i>
                  </div>
                </th>
                <th scope="col" class="py-3 text-center" style="min-width: 170px;">
                  {{ labels.linkedMeetings || 'Meetings' }}
                </th>
            ...
            <tbody>
              ...
              <tr v-else v-for="item in rows" :key="item.id" class="table-data-row">
                ...
                <td class="ps-4 py-3">
                  <div class="d-flex align-items-center gap-2">
                    <div class="topic-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 38px; height: 38px; background: rgba(13, 110, 253, 0.12); color: #0d6efd;">
                ...
                <td class="py-3 text-center">
                  <span class="badge rounded-pill px-3 py-2 fw-semibold" :class="item.meetings_count > 0 ? 'bg-info-subtle text-info' : 'bg-secondary-subtle text-secondary'">
            ...
            >
              <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-start mb-2">
                  <div class="d-flex align-items-center gap-2">
                    <div class="topic-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 36px; height: 36px; background: rgba(13, 110, 253, 0.12); color: #0d6efd;">
                      <i class="bi bi-chat-quote"></i>
                    </div>
                    <div>
                  ...
                  <span class="badge rounded-pill px-2 py-1 small" :class="item.meetings_count > 0 ? 'bg-info-subtle text-info' : 'bg-secondary-subtle text-secondary'">
                    <i class="bi bi-calendar-event me-1"></i>
                    {{ item.meetings_count || 0 }}
                  </span>
                </div>
            ...
            <div class="d-flex align-items-center gap-2">
              ...
              <div>
                <h5 class="modal-title fw-bold mb-0" style="color: var(--text-primary);">
                  {{ labels.topicDetails || 'Topic Details' }}
                </h5>
            ...
            <div v-if="selectedItem" class="row g-3">
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.topicNameAr || 'Arabic Name' }}</div>
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.topicNameEn || 'English Name' }}</div>
                  <div class="fw-bold fs-6" style="color: var(--text-primary);">
              ...
              <div class="col-12">
                <div class="p-3 rounded-3 detail-info-box d-flex justify-content-between align-items-center">
                  <div>
                    <div class="small text-muted fw-semibold mb-1">{{ labels.linkedMeetings || 'Linked Meetings' }}</div>
                    <div class="fw-bold fs-5 text-info">
                      {{ selectedItem.meetings_count || 0 }}
                    </div>
                  </div>
...
const kpiData = reactive({
  total_topics: props.initialKpiStats?.total_topics || 0,
  total_meetings: props.initialKpiStats?.total_meetings || 0,
  standard_topics: props.initialKpiStats?.standard_topics || 0
});
...
const hasActiveFilters = computed(() => {
  return searchQuery.value !== '' || selectedHasMeetings.value !== '';
});
...
const fetchData = async () => {
  ...
  try {
    ...
    if (selectedHasMeetings.value) {
      params.has_meetings = selectedHasMeetings.value;
    }
...
const confirmDelete = async (item) => {
  const confirmMsg = (props.labels.confirmDelete || 'Are you sure you want to delete topic ":name"?')
  ...
  if (result.isConfirmed) {
    try {
      ...
      if (res.data?.success || res.status === 200 || res.status === 204) {
        Swal.fire({
          icon: 'success',
          title: props.labels.deletedSuccess || 'Topic deleted successfully.',
          timer: 2000,
          showConfirmButton: false
        });
        kpiData.total_topics = Math.max(0, kpiData.total_topics - 1);
        fetchData();
      }
    } catch (err) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: err.response?.data?.message || 'Failed to delete topic.'
      });
    }
  }
};
