<template>
  <div class="neighborhoods-container" :dir="isRtl ? 'rtl' : 'ltr'">
      ...
      <div class="col-12 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.totalNeighborhoods || 'Total Neighborhoods' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.total_neighborhoods }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-6 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.citiesRepresented || 'Cities Represented' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.total_cities }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-6 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.hostedGroups || 'Hosted Groups' }}</div>
    ...
    <div class="card border-0 shadow-sm mb-4 glass-card">
      <div class="card-body p-3 d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div class="d-flex flex-wrap align-items-center gap-2 flex-grow-1">
          ...
          <div class="search-box-wrapper">
            <div class="search-box">
              ...
              <input
                ...
                v-model="searchQuery"
                class="search-input"
                :placeholder="labels.searchPlaceholder || 'Search neighborhoods...'"
                @input="onSearchInput"
              />
              <button
                v-if="searchQuery"
          ...
          <!-- City Filter -->
          <div class="filter-wrapper" v-if="cities.length > 0">
            <select v-model="selectedCity" @change="fetchData" class="form-select form-select-sm filter-select">
              <option value="">{{ labels.allCities || 'All Cities' }}</option>
          ...
          <div class="filter-wrapper">
            <select v-model="selectedHasGroups" @change="fetchData" class="form-select form-select-sm filter-select">
              <option value="">{{ labels.hasMeetings || 'Has Groups' }}</option>
              <option value="yes">{{ labels.withMeetings || 'With Groups' }}</option>
              <option value="no">{{ labels.withoutMeetings || 'Without Groups' }}</option>
            </select>
          </div>
          ...
          <table class="table table-hover align-middle mb-0 custom-datatable">
              ...
              <tr class="table-header-row">
                <th scope="col" class="ps-4 py-3" style="min-width: 250px;" @click="setSort('ar_name')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.neighborhoodNameAr || 'Neighborhood (AR)' }}</span>
                    <i class="bi sort-icon" :class="getSortIcon('ar_name')"></i>
                  </div>
                </th>
                <th scope="col" class="py-3" style="min-width: 230px;" @click="setSort('en_name')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.neighborhoodNameEn || 'Neighborhood (EN)' }}</span>
            ...
            <tbody>
              ...
              <tr v-else v-for="item in rows" :key="item.id" class="table-data-row">
                <!-- Neighborhood Name (AR) -->
                <td class="ps-4 py-3">
                  <div class="d-flex align-items-center gap-2">
                    <div class="neighborhood-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 38px; height: 38px; background: rgba(13, 202, 240, 0.12); color: #0dcaf0;">
                ...
                <!-- City -->
                <td class="py-3">
                  <span v-if="item.city" class="badge rounded-pill bg-light text-dark border px-3 py-2 fw-semibold">
                    <i class="bi bi-buildings me-1 text-primary"></i>
                    {{ isRtl ? (item.city.ar_name || item.city.en_name) : (item.city.en_name || item.city.ar_name) }}
                  </span>
            ...
            >
              <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-start mb-2">
                  <div class="d-flex align-items-center gap-2">
                    <div class="neighborhood-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 36px; height: 36px; background: rgba(13, 202, 240, 0.12); color: #0dcaf0;">
                      <i class="bi bi-geo-alt"></i>
                    </div>
                    <div>
                ...
                <div v-if="item.city" class="mb-2 small text-muted">
                  <i class="bi bi-buildings me-1 text-primary"></i>
                  {{ isRtl ? (item.city.ar_name || item.city.en_name) : (item.city.en_name || item.city.ar_name) }}
                </div>
            ...
            <div class="d-flex align-items-center gap-2">
              ...
              <div>
                <h5 class="modal-title fw-bold mb-0" style="color: var(--text-primary);">
                  {{ labels.neighborhoodDetails || 'Neighborhood Details' }}
                </h5>
            ...
            <div v-if="selectedItem" class="row g-3">
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.neighborhoodNameAr || 'Arabic Name' }}</div>
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.neighborhoodNameEn || 'English Name' }}</div>
                  <div class="fw-bold fs-6" style="color: var(--text-primary);">
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.city || 'City' }}</div>
                  <div class="fw-bold fs-6" style="color: var(--text-primary);">
                    {{ selectedItem.city ? (isRtl ? (selectedItem.city.ar_name || selectedItem.city.en_name) : (selectedItem.city.en_name || selectedItem.city.ar_name)) : 'N/A' }}
                  </div>
                </div>
              </div>
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.hostedGroups || 'Hosted Groups' }}</div>
...
const selectedHasGroups = ref('');

// KPIs
const kpiData = reactive({
  total_neighborhoods: props.initialKpiStats?.total_neighborhoods || 0,
  total_cities: props.initialKpiStats?.total_cities || 0,
...
const hasActiveFilters = computed(() => {
  return searchQuery.value !== '' || selectedCity.value !== '' || selectedHasGroups.value !== '';
});

const resetAllFilters = () => {
  searchQuery.value = '';
  selectedCity.value = '';
  selectedHasGroups.value = '';
  currentPage.value = 1;
  fetchData();
};
...
const fetchData = async () => {
  ...
  try {
    ...
    if (selectedHasGroups.value) {
      params.has_groups = selectedHasGroups.value;
    }
...
const confirmDelete = async (item) => {
  const confirmMsg = (props.labels.confirmDelete || 'Are you sure you want to delete neighborhood ":name"?')
  ...
  if (result.isConfirmed) {
    try {
      ...
      if (res.data?.success || res.status === 200 || res.status === 204) {
        Swal.fire({
          icon: 'success',
          title: props.labels.deletedSuccess || 'Neighborhood deleted successfully.',
          timer: 2000,
          showConfirmButton: false
        });
        kpiData.total_neighborhoods = Math.max(0, kpiData.total_neighborhoods - 1);
        fetchData();
      }
    } catch (err) {
