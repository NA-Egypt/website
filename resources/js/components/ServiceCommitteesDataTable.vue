<template>
  <div class="service-committees-container" :dir="isRtl ? 'rtl' : 'ltr'">
      ...
      <div class="col-12 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.totalCommittees || 'Total Committees' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.total_committees }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-6 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.childWorkgroups || 'Child Workgroups' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.child_workgroups_count }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-6 col-md-4">
        <div class="card border-0 shadow-sm kpi-card h-100">
          <div class="card-body p-3 d-flex align-items-center gap-3">
            ...
            <div>
              <div class="kpi-label text-muted small fw-medium">{{ labels.activeCommittees || 'Active Committees' }}</div>
              <div class="kpi-value fs-4 fw-bold" style="color: var(--text-primary);">{{ kpiData.active_committees }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="card border-0 shadow-sm mb-4 glass-card">
      <div class="card-body p-3 d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div class="d-flex flex-wrap align-items-center gap-2 flex-grow-1">
          ...
          <div class="search-box-wrapper">
            <div class="search-box">
              ...
              <input
                ...
                v-model="searchQuery"
                class="search-input"
                :placeholder="labels.searchPlaceholder || 'Search committees...'"
                @input="onSearchInput"
              />
              <button
                v-if="searchQuery"
          ...
          <!-- Has Workgroups Filter -->
          <div class="filter-wrapper">
            <select v-model="selectedHasWorkgroups" @change="fetchData" class="form-select form-select-sm filter-select">
              <option value="">{{ labels.hasWorkgroups || 'Has Workgroups' }}</option>
              <option value="yes">{{ labels.withWorkgroups || 'With Workgroups' }}</option>
              <option value="no">{{ labels.withoutWorkgroups || 'Without Workgroups' }}</option>
            </select>
          </div>
          ...
          <table class="table table-hover align-middle mb-0 custom-datatable">
            <thead>
              <tr class="table-header-row">
                <th scope="col" class="ps-4 py-3" style="min-width: 250px;" @click="setSort('ar_name')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.committeeNameAr || 'Committee (AR)' }}</span>
                    <i class="bi sort-icon" :class="getSortIcon('ar_name')"></i>
                  </div>
                </th>
                <th scope="col" class="py-3" style="min-width: 250px;" @click="setSort('en_name')">
                  <div class="d-flex align-items-center gap-1 cursor-pointer">
                    <span>{{ labels.committeeNameEn || 'Committee (EN)' }}</span>
                ...
                <th scope="col" class="py-3 text-center" style="min-width: 160px;">
                  {{ labels.childWorkgroups || 'Workgroups' }}
                </th>
            ...
            <tbody>
              ...
              <tr v-else v-for="item in rows" :key="item.id" class="table-data-row">
                ...
                <td class="ps-4 py-3">
                  <div class="d-flex align-items-center gap-2">
                    <div class="committee-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 38px; height: 38px; background: rgba(13, 110, 253, 0.12); color: #0d6efd;">
                ...
                <!-- Child Workgroups Count -->
                <td class="py-3 text-center">
                  <span class="badge rounded-pill px-3 py-2 fw-semibold" :class="item.workgroups_count > 0 ? 'bg-info-subtle text-info' : 'bg-secondary-subtle text-secondary'">
                    <i class="bi bi-diagram-3 me-1"></i>
                    {{ item.workgroups_count || 0 }}
                  </span>
                </td>
            ...
            >
              <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-start mb-2">
                  <div class="d-flex align-items-center gap-2">
                    <div class="committee-avatar rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm" style="width: 36px; height: 36px; background: rgba(13, 110, 253, 0.12); color: #0d6efd;">
                      <i class="bi bi-diagram-2"></i>
                    </div>
                    <div>
                      <div class="fw-bold" style="color: var(--text-primary);">{{ item.ar_name }}</div>
                      <div class="text-muted small">{{ item.en_name }}</div>
                    </div>
                  </div>
                  <span class="badge rounded-pill px-2 py-1 small" :class="item.workgroups_count > 0 ? 'bg-info-subtle text-info' : 'bg-secondary-subtle text-secondary'">
                    <i class="bi bi-diagram-3 me-1"></i>
                    {{ item.workgroups_count || 0 }}
                  </span>
                </div>
            ...
            <div class="d-flex align-items-center gap-2">
              ...
              <div>
                <h5 class="modal-title fw-bold mb-0" style="color: var(--text-primary);">
                  {{ labels.committeeDetails || 'Committee Details' }}
                </h5>
            ...
            <div v-if="selectedItem" class="row g-3">
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.committeeNameAr || 'Arabic Name' }}</div>
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100">
                  <div class="small text-muted fw-semibold mb-1">{{ labels.committeeNameEn || 'English Name' }}</div>
                  <div class="fw-bold fs-6" style="color: var(--text-primary);">
              ...
              <div class="col-12 col-md-6">
                <div class="p-3 rounded-3 detail-info-box h-100 d-flex justify-content-between align-items-center">
                  <div>
                    <div class="small text-muted fw-semibold mb-1">{{ labels.childWorkgroups || 'Workgroups' }}</div>
                    <div class="fw-bold fs-5 text-info">
                      {{ selectedItem.workgroups_count || 0 }}
                    </div>
                  </div>
...
const selectedHasWorkgroups = ref('');

// KPIs
const kpiData = reactive({
  total_committees: props.initialKpiStats?.total_committees || 0,
  child_workgroups_count: props.initialKpiStats?.child_workgroups_count || 0,
  active_committees: props.initialKpiStats?.active_committees || 0
});
...
const hasActiveFilters = computed(() => {
  return searchQuery.value !== '' || selectedHasWorkgroups.value !== '';
});

const resetAllFilters = () => {
  searchQuery.value = '';
  selectedHasWorkgroups.value = '';
  currentPage.value = 1;
  fetchData();
};
...
const fetchData = async () => {
  ...
  try {
    ...
    if (selectedHasWorkgroups.value) {
      params.has_workgroups = selectedHasWorkgroups.value;
    }
...
const confirmDelete = async (item) => {
  const confirmMsg = (props.labels.confirmDelete || 'Are you sure you want to delete committee ":name"?')
  ...
  if (result.isConfirmed) {
    try {
      ...
      if (res.data?.success || res.status === 200 || res.status === 204) {
        Swal.fire({
          icon: 'success',
          title: props.labels.deletedSuccess || 'Committee deleted successfully.',
          timer: 2000,
          showConfirmButton: false
        });
        kpiData.total_committees = Math.max(0, kpiData.total_committees - 1);
        fetchData();
      }
    } catch (err) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: err.response?.data?.message || 'Failed to delete committee.'
      });
    }
  }
};
